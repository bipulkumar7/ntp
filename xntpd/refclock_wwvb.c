 ...
 *  C      AQY  D   T
 *	poll: 'T'; offsets: T = 9, Y = 2, D = 5, A = 0, Q = 1
 *	i = synchronization flag (' ' = in synch, '?' = out synch)
 ...
 * at Q.
 *
 * The Q is normally ' ' when the time error is less than 1 ms and and
 * a character in the set 'A'...'D' when the time error is less than 10,
 * 100, 500 and greater than 500 ms respectively. The L is normally
 * ' ', but is set to 'L' early in the month of an upcoming UTC leap
 * second and reset to ' ' on the first day of the following month. The
 * D is set to 'S' for standard time 'I' on the day preceding a switch
 * to daylight time, 'D' for daylight time and 'O' on the day preceding
 * a switch to standard time. The start bit of the first <cr> is
 ...
 * internally by the clock. If flag4 of the fudge configuration command
 * is set to 1, the driver will retrieve this table and write it to the
 * clockstats file on when the first timecode message of a new day is
 * received.
 */
	...
	struct recvbuf *rbufp;
{
	...
	switch (pp->lencode) {
		...
		/*
	 	 ...
	 	 */
		up->qualchar = up->leapchar = ' ';
		if (sscanf(pp->lastcode, "%c %3d %2d:%2d:%2d",
		    &up->syncchar, &pp->day, &pp->hour, &pp->minute,
		    &pp->second) == 5)
			break;
		...
		if (sscanf(pp->lastcode, "%c%c %2d %3d %2d:%2d:%2d.%3d %c",
		    &up->syncchar, &up->qualchar, &pp->year,
		    &pp->day, &pp->hour, &pp->minute, &pp->second,
		    &pp->msec, &up->leapchar) == 9)
			break;
