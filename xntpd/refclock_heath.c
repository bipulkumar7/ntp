 ...
 * MANUAL mode and the current year. In MANUAL mode the clock responds
 * to a rising edge of the RTS modem control line by sending the
 * timecode. The external DIPswitches should be set to GMT and 24-hour
 * format.
 *
 ...
 *
 * It is very important that the year be set correctly in the
 * DIPswitches. Otherwise, the day of year will be incorrect after 28
 * April of a normal or leap year.
 */
	...
	struct peer *peer;
{
	 ...
	 */
	ioctl(pp->io.fd, TIOCMBIC, (char *)&bits);
	gettstamp(&up->tstamp);
