...
s_fp	sys_bdelay;		/* default offset for broadcast client */
	...
	register struct peer *peer;
{
	...
	} else if (peer->flags & FLAG_SYSPEER && peer->hpoll > sys_poll)
		peer->hpoll = sys_poll;
	if (peer->hpoll < peer->minpoll)
		peer->hpoll = peer->minpoll;
	...
	struct recvbuf *rbufp;
{
	...
	case MODE_MCLIENT:
		/*
		 * This mode is temporary and does not appear outside this
		 * routine. It lasts only from the time the broadcast/
		 * multicast is recognized until the association is
		 * instantiated.
		 */
		switch (hismode) {
		case MODE_BROADCAST:
	...
	int trustable;			/* used as "valid header" */
{
	...
	/*
	 * If running in a client/server association, calculate the clock
	 * offset c, roundtrip delay d and dispersion e. We use the equations
	 * (reordered from those in the spec). Note that, in a broadcast
	 * association, org has been set to the time of last reception.
	 *
	 * c = ((t2 - t3) + (t1 - t0)) / 2
	 * d = (t2 - t3) - (t1 - t0)
	...
	L_SUB(&t23, &p_org);
	ci = t10;
	ei = (FP_SECOND >> (-(int)sys_precision)) + peer->rec.l_ui - p_org.l_ui;

	/*
	 * If running in a broacast association, the clock offset is (t1 - t0)
	 * corrected by the one-way delay, but we can't measure that directly;
	 * therefore, we start up in client/server mode, calculate the clock
	 * offset, using the engineered refinement algorithms, while also
	 * receiving broadcasts. When a broadcast is received in client/server
	 * mode, we calculate a correction factor to use after switching back
	 * to broadcast mode. We know NTP_SKEWFACTOR == 16, which accounts for
	...
	struct peer *peer;
{
	...
	/*
	 * Call the clock selection algorithm to see if this update causes
	 * the peer to change.
	 */
	clock_select();

	/*
	 * Quit if this peer isn't the system peer or is too far away.
	 * Other peers * used in the combined offset are not allowed to set
...
int default_get_precision()
{
	...
	syslog(LOG_INFO, "precision = %d", val);
	if (usec >= HUSECS)
		val = MINSTEP;	/* val <= MINSTEP; fast machine */
	for (i = 0, diff = HUSECS; diff > val; i--, diff >>= 1);
	return i;
}
