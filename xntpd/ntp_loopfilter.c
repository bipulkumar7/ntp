...
static	long last_time;		/* time of last clock adjustment (s) */
	u_fp clock_stability;	/* measured clock stability (scaled ppm) */
	s_fp drift_comp;	/* measured clock frequency (scaled ppm) */
...
static	u_long	pps_time;	/* last pps sample time */
	...
	int pps_enable;		/* true if pps enabled */
 ...
 */
	 ...
	 */
	fdpps = refclock_open(PPS_DEV, PPS_BAUD, LDISC_CLK))
	...
	struct peer *peer;		/* from peer - for messages */
{
	...
	/*
	 * Calculate frequency stability. This is not in the NTPv3 spec,
	 * which is the only excuse for using the uglification. We
	 * need this for the multicast stuff. The result is scaled in
	 * ppm.
	 */
	if (!first_adjustment) {
		tmp = (offset - last_adjustment) << 2;
		if (tmp < 0)
			tmp = -tmp;
		tmp = (tmp / (current_time - last_time)) - clock_stability;
		if (tmp < 0)
			clock_stability -= -tmp >> STAB_AVG;
		else
			clock_stability += tmp >> STAB_AVG;
	}
...
adj_host_clock()
{
	...
	if (pps_update && current_time - pps_update > PPS_MAXUPDATE)
		pps_update = 0;
	if (pps_time && pps_update && pps_enable) {
		if (!pps_control)
	...
	int int_value;
{
	...
	case LOOP_PPSBAUD:
		if (fdpps == -1)
			break;
		(void)fclose(fdpps);
		fdpps = refclock_open(PPS_DEV, SYS_BAUD(int_value))
		io.fd = fdpps;
		break;
#endif /* PPSCLK */
